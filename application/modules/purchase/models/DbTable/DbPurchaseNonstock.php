...
class Purchase_Model_DbTable_DbPurchaseNonstock extends Zend_Db_Table_Abstract
{	
	...
	public function addPurchaseOrder($data)
	{
		...
		try{
			...
			$this->_name="tb_purchase_nonstock";
			...
			foreach ($ids as $i)
			{
				...
				$this->_name='tb_purchase_nonstock_item';
				$this->insert($data_item);
			}
			$db->commit();
		}catch(Exception $e){
	...
	public function updatePurchaseOrder($data)
	{
		...
		try{
			...
			$sql = "DELETE FROM `tb_vendor_payment` 
                    WHERE tb_vendor_payment.id=
			(SELECT vd.receipt_id FROM `tb_vendorpayment_detail` AS  vd WHERE vd.`invoice_id`=$id)";
			$db->query($sql);
	...
	public function getPurchaseDetailById($id){//just new
		$db=$this->getAdapter();
		$sql = "SELECT * FROM `tb_purchase_nonstock_item` WHERE purchase_id=$id ";
